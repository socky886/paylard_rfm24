...
//	spi_rw_reg(W_REGISTER+EN_RXADDR,0x01); 							//使能通道0的接收地址  
	...
	spi_rw_reg(W_REGISTER + FEATURE, 0x06);      					// 使能动态负载长度,Disable command W_TX_PAYLOAD_NOACK
...
//txbuf:待发送数据首地址
//返回值:发送完成状况
uchar Si24R1_TxPacket(uchar *txbuf, uchar txLen, uchar *rxbuf, uchar *rxLen)
{
	uchar sta;
	CE = 0;
	spi_rw_reg(FLUSH_TX, 0x00);//清除TX FIFO
  	spi_write_buf(W_TX_PAYLOAD,txbuf,txLen);//写数据到TX BUF  
 	CE=1;//启动发送	   
	...
	if(sta&TRX_OK)                  // 发送成功，且接收到ACK(with PAYLOAD)
	{
		if(sta & RXFIFO_EMPTY)	// RX FIFO 为空(接收错误)
		{
			return 0;
		}
		else
		{
			*rxLen = spi_rd_reg(R_RX_PL_WID);	// 获取数据包长度(动态负载)
			spi_read_buf(R_RX_PAYLOAD, rxbuf, *rxLen);//读取数据
			spi_rw_reg(FLUSH_RX, 0x00);//清除RX FIFO
			spi_rw_reg(FLUSH_TX, 0x00);//清除TX FIFO
			return sta; 
		}
	}
...
//txbuf:待发送数据首地址
//返回值:0，接收完成；其他，错误代码
uchar Si24R1_RxPacket(uchar *rxbuf, uchar *rxLen)
{
	uchar sta;
	CE = 1;		//此句不可缺少
	sta=spi_rd_reg(STATUS);             //读取状态寄存器的值    	 
	spi_rw_reg(W_REGISTER+STATUS,sta);  //清除TX_DS或MAX_RT中断标志
	if(g_cFirstPacketFlag)              // 非第一包数据，产生RX_DR和TX_DS
	{
		if(sta&TRX_OK)//接收到数据
		{
			if(sta & RXFIFO_EMPTY)	// RX FIFO 为空(接收错误)
